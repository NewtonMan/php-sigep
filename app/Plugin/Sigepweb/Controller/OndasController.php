...
class OndasController extends SigepwebAppController {
    
    public $uses = ['Sigepweb.SigepConta', 'Sigepweb.Encomenda', 'Separacao.Movimento', 'Separacao.MovimentoSeparacao', 'Separacao.MovimentoSeparacaoMaterial'];
    
    public function etiquetas_old_lento($onda_id) {
        Configure::write('debug', 2);
        $onda = $this->MovimentoSeparacao->read(null, $onda_id);
        $conta = $this->SigepConta->find('first', ['conditions' => [
            'SigepConta.cliente_id' => $onda['MovimentoSeparacao']['cliente_id'],
        ]]);
        ...
        //if (!file_exists($arqPDF)){
            ...
            return $this->redirect('/sigepweb/ondas/encomendas/' . implode('/', $encomendas));
        //}
    ...
    public function etiquetas($onda_id) {
        ...
        return $this->redirect("/sigepweb/encomenda/etiquetas/" . implode('/', $encomendas));
    }
